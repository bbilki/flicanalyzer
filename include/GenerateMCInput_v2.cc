...
// 	float tlims[5][2]={{1100,2040},{1100,2040},{1100,2040},{1150,2100},{1440,2350}};
	float tlims[5][2]={{1300,2040},{1050,2040},{1050,2040},{1150,2100},{1440,2350}};
...
// 	sprintf(hname,"MCTrackParameters_%d_1.txt",PRunNo);
	sprintf(hname,"MCTrackParameters_%d_2.txt",PRunNo);
...
// 			
// 			outfile<<RunNo<<" "<<aa<<" "<<((bb[0]-63.5)*2.54)<<" "<<((bb[1]-63.5)*2.54)<<" "<<bb[2]<<" "<<((bb[3]-63.5)*2.54)<<" "<<((bb[4]-63.5)*2.54)<<" "<<bb[5]<<endl;
// 		}
// 		infile.close();
		...
		while(!infile.eof())
		{
			infile>>mce.E>>mce.XYT[0][0]>>mce.XYT[0][1]>>mce.XYT[0][2]>>mce.XYT[1][0]>>mce.XYT[1][1]>>mce.XYT[1][2];
			mce.XYT[0][0]=((mce.XYT[0][0]-63.5)*2.54);
			mce.XYT[0][1]=((mce.XYT[0][1]-63.5)*2.54);
			mce.XYT[1][0]=((mce.XYT[1][0]-63.5)*2.54);
			mce.XYT[1][1]=((mce.XYT[1][1]-63.5)*2.54);
			MCE.push_back(mce);
		}
		infile.close();
		MCE.pop_back();
		for(int i1=0;i1<MCE.size();i1++)
		{
			outfile<<MCE[i1].R<<" "<<MCE[i1].E<<" "<<MCE[i1].XYT[0][0]<<" "<<MCE[i1].XYT[0][1]<<" "<<MCE[i1].XYT[0][2]<<" "<<MCE[i1].XYT[1][0]<<" "<<MCE[i1].XYT[1][1]<<" "<<MCE[i1].XYT[1][2]<<endl;
		}
	}
	outfile.close();
}