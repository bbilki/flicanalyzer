void HitAnalysis4()
{
	...
	TH2F* WireSpan_vs_TimeSpan_Col=new TH2F("WireSpan_vs_TimeSpan_Col","WireSpan_vs_TimeSpan_Col",4096,-0.5,4095.5,128,-0.5,127.5);
			...
			for(int i2=0;i2<4096;i2++)
			{
				if(hh[i1]->GetBinContent(i2+1)>5.*BLsigmas[i1])
				{
					...
					if(rightbin==-1)
					{
						for(int is1=i2;is1<4095;is1++)
						{
							if(hh[i1]->GetBinContent(is1+1)>hh[i1]->GetBinContent(is1+2))
							{
								rightbin=is1+1;
								break;
							}
					...
					if(leftbin==-1)
					{
						for(int is1=i2;is1>=2;is1--)
						{
							if(hh[i1]->GetBinContent(is1+1)<hh[i1]->GetBinContent(is1))
							{
								leftbin=is1+1;
								break;
							}
...
// 		cout<<I<<" "<<nww1peak<<" "<<nwwmorepeaks<<" "<<(((float)nww1peak)/((float)(nww1peak+nwwmorepeaks)))<<" "<<isMIP<<endl;
		...
		if(isMIP)
		{
			int wminmax[2]={1000,0};
			float tminmax[2]={1000,0};
			for(int i1=0;i1<128;i1++)
			{
...
// 					if(rightbin==-1) rightbin=4096;
					if(leftbin!=-1 && rightbin!=-1)
					{
						...
						PP.push_back(pp);
						if(i1<wminmax[0]){wminmax[0]=i1;}
						if(i1>wminmax[1]){wminmax[1]=i1;}
						if(pp.t<tminmax[0]){tminmax[0]=pp.t;}
						if(pp.t>tminmax[1]){tminmax[1]=pp.t;}
					}
				}
			}
			...
			WireSpan_vs_TimeSpan_Col->Fill(tminmax[1]-tminmax[0],wminmax[1]-wminmax[0]);
		}
		if(I<50)
		{
			outroot->cd();
...
// 		if(I>50) break;
	...
	HitFWHM->Write();
	WireSpan_vs_TimeSpan_Col->Write();
